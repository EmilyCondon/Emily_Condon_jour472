...
pg_county <- read_rds("pg_county.rds")
...
```{r}
attendance <- read_rds("attendance.rds")
...
pg_tract_income <- read_rds("pg_tract_income.rds")
...
Two questions with my parking data are:
1. To understand population density that affects hospital Emergency Department demand: What is the population density across Maryland and add plots of each hospital?
  -For this I would need the Maryland census tracts, the state outline frame and locations of each hopsital.
2. For Baltimore hospitals - what is the closet hospital for people in each area? Assign each hospital a color and then color code the map of Baltimore for the closest geographic areas.
  -For this I would again need hospital locations, an outline of Baltimore and then geocoded locations for each community and some sort of equation that measured distance from hospitals to each neighborhood.
