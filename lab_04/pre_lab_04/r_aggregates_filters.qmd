...
```{r}
***Note: below made code to find the total amount of tracts, which came out to 200***
```
...
In 2020, almost half of @BaltCityHall neighbors fell below a median income of $50k. 

In 2016, almost half of Baltimore neighborhoods fell below a median income of $40k. 

While the median income increased... do you think this is enough money for Baltimoreans to live off of?
...
baltcity_income %>%
  select(Neighborhood, x2016) %>%
  filter(x2016 < 39583) %>%
  summarise(
  count_tracts = n())

Baltcity2016_below_median <- (98/200)*100
```

2016: 49% was below $40k median
...
sum(baltcity_race$x2020_total, na.rm = TRUE)

pct_white <- baltcity_race %>%
  select(census_tract, x2020_white, x2020_total) %>%
  mutate(pct_white= (x2020_white/x2020_total)) %>%
...
pct_white %>%
  summarize(sum(pct_white, na.rm = TRUE))
