...
#Answer: 2018 had the most arrests at 159. 2016 has 95, 2017 had 79 and 2019 had 108.
...
arrest %>%
  ...
  coord_flip() + 
  geom_col()+
  geom_text(aes(label=n), hjust = -.5, size = 2.5) +
    labs(
    title="Arrests, San Francisco",
    x = "disposition",
    y = "year",
    caption = "source: SF PD Service Call Data, 2016-2019")
...
arrest %>%
  ggplot() +
  geom_bar(aes(x=year, n=disposition)) +
  geom_bar(aes(x=2016), color = "blue") +
  geom_bar(aes(x=2017), color = "brown") +
  geom_bar(aes(x=2018), color = "green") +
  geom_bar(aes(x=2019), color = "yellow") 
...
all_dispo <- SF %>%
  select(disposition) %>%
  filter(disposition != "Not recorded") %>% 
  count(disposition) %>%
...
```{r}
top_12_dispo <- all_dispo %>%
  ggplot(aes(x=disposition, y=n, weight=n, fill=n)) +
  geom_col()+
  geom_text(aes(label=n), vjust = -1, size = 2.5) +
    labs(
    title="Most Common 311 Call Resolutions",
    x = "Resolution",
    y = "Count",
    caption = "source: SF PD Service Call Data, 2016-2019")
...
#Question for Prof. Wells - Why does by n color chart have overlapping numbers?
```
