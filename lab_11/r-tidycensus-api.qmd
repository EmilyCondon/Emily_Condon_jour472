...
male2 <- male %>%
  inner_join(total_pop_2020, by=c("state")) 

male_pct <- male2 %>% 
  filter(variable == "male_population") %>% 
  mutate(male_pct = estimate/total_population*100) %>% 
  arrange(desc(male_pct))

#The state with the highest percentage of men was at Alaska with 52.19%
```
