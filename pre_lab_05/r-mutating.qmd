...
```{r include=FALSE}
#ppp_applications_md_small <- read_csv("ppp_applications_md.csv") %>%
#  select(name, amount, city, servicing_lender_state)

#write_csv(ppp_applications_md,"ppp_applications_md.csv")
...
```{r}
# Beaver Creek had the highest absolute change of $118,292. Shaft had the highest percent change at 399.88%.
```
...
#The first quartile median income for 2020 was $63,327.  
```
```{r}
category <- city_md_income %>%
  mutate(
    category = case_when(
...
```{r}
#At the end of the code in the chunk above, I would add a slice max one through then and slice min one through ten.
```
